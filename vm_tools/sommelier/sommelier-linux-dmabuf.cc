...
static void send_format_table(struct sl_host_linux_dmabuf_feedback *feedback,
  ...
  for (int i = 0; i < format_count; ++i) {
    ...
    ret = write(fd, &format_table_item, sizeof(format_table_item));
    if (ret != sizeof(format_table_item)) {
      perror("failed to write format table");
    }
  }
...
static int advertise_feedback_event(void* data) {
  ...
  // Send main device
  dev_t devid;
  if (!devid_from_fd(ctx->drm_fd, &devid)) {
...
static void sl_linux_dmabuf_modifier(void* data,
  ...
  if (host->version >= ZWP_LINUX_DMABUF_V1_MODIFIER_SINCE_VERSION &&
      host->version < ZWP_LINUX_DMABUF_V1_GET_DEFAULT_FEEDBACK_SINCE_VERSION) {
